...
bool Receiver::start(CallableObject& co)
{
	return listener.start(co);
}
...
void Sender::start()
{
	std::function <void()> threadProc = [&]() {
		while (true)
		{
			...
			if (msg.to().address != lastEP.address || msg.to().port != lastEP.port || msg.containsKey("connect"))
			{
				connecter.shutDown();
				//connecter.close();
